...
if __name__ == '__main__':
    ...
    model = GPT2PromptTuningLM.from_pretrained(
        'gpt2',
        tokeniser,
        prompt_str='[TRANSLATE_EN_DE]'
    ).to(device)
    
    print('tokenising train file')
    train_file_1 = 'dataset/train_en.txt'
    train_file_2 = 'dataset/train_de.txt'
    dataset = CSVDataset(train_file_1, train_file_2,tokeniser, max_length=768, subset_size=args.subset_size)
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True)
    
    print('tokenising val file')
    validation_file_1 = 'dataset/val_en.txt'
    validation_file_2 = 'dataset/val_de.txt'
    dataset_val = CSVDataset(validation_file_1,validation_file_2, tokeniser, max_length=768, subset_size=int(args.subset_size/10))
    dataloader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=True)
    
    print('tokeinsing test file')
    test_file_1 = 'dataset/test_en.txt'
    test_file_2 = 'dataset/test_de.txt'
    dataset_test = CSVDataset(test_file_1,test_file_2, tokeniser, max_length=768, subset_size=int(args.subset_size/10))
    ...
    if args.is_train:
        ...
        print('saving model')
        torch.save(model.state_dict(), f'../models/mt.pth')
    else:
        print('evaluating model')
        model.load_state_dict(torch.load(f'../models/mt.pth'))
        
        print('evaluating train set')
        test_loop(model, dataloader, tokeniser)
        
        print('evaluating test set')
        test_loop(model, dataloader_test, tokeniser)
